...
Разбивает оригинальный файл БД по год - месяц - ДЕНЬ
...
# todo: ПОПЫТАТЬСЯ НЕ РАЗБИВАТЬ НА ФИЗИЧЕСКИЕ ФАЙЛЫ
...
def new_date():
    ...
    new_orig_copy.to_csv('C:/Users/Egor/Desktop/all_parameters_okhotskoe/new_orig_copy.csv', index=False)
    print('OK')


def cutter_orig_file():
    """
    РАЗБИВАЕТ ОРИГИНАЛ ФАЙЛА НА НЕСКОЛЬКО ПО ГОДУ И МЕСЯЦУ и ДНЮ!!!!!!!!!!!!!!
    """
    new_df = pd.read_csv('C:/Users/Egor/Desktop/Oxigen_result/All_parameters_without_none.csv', delimiter=',')
    for year in range(1930, 2016):
        if year in list(new_df['Year']):
            for month in range(1, 13):
                if month in list(new_df.query('Year == @year')['Month']):
                    for day in range(1,32):
                        if day in list(new_df.query('Day == @day')['Day']):
                            df = new_df.query('Year == @year & Month == @month & Day == @day')
                            df.to_csv('C:/Users/Egor/Desktop/Oxigen_result/cutter/'+f'{year}_{month}_{day}.csv', index=False)
...
def cleaning_cuttered_files():
    ...
    for year in range(1930, 2016):
        if year in list(new_df['Year']):
            for month in range(1, 13):
                if month in list(new_df.query('Year == @year')['Month']):
                    path_lonely_csv = 'C:/Users/Egor/Desktop/all_parameters_okhotskoe/cutter/' + f'{year}_{month}.csv'
    ...
    for year in range(1930, 2016):
        if year in list(new_df['Year']):
            for month in range(1, 13):
                if month in list(new_df.query('Year == @year')['Month']):
                    for day in range(1, 32):
                        if day in list(new_df.query('Year == @year')['Day']):
                            path_df_new = 'C:/Users/Egor/Desktop/Oxigen_result/cutter/' + f'{year}_{month}_{day}.csv'
                            df = pd.read_csv(path_df_new, delimiter=',')
                            # todo: УДАЛЯЮ ДУБЛИКАТЫ В КАЖДОМ ОТДЕЛЬНОМ ДНЕ
                            df = df.drop_duplicates(['long', 'lat', 'level'])   # Удаляю дубликате в дне
                            ...
                            path_df_with_nst = 'C:/Users/Egor/Desktop/Oxigen_result/cutter/nst/' + f'{year}_{month}_{day}_nst.csv'
...
def joiner_clean_files():
    ...
    path_last = 'C:/Users/Egor/Desktop/Oxigen_result/joined/1930_6_5_nst.csv'
    df_last = pd.read_csv(path_last, header=0, delimiter=',')

    new_orig_copy_df = pd.read_csv('C:/Users/Egor/Desktop/Oxigen_result/All_parameters_without_none.csv', delimiter=',')
    for year in range(1930, 2016):
        if year in list(new_orig_copy_df['Year']):
            for month in range(1, 13):
                if month in list(new_orig_copy_df.query('Year == @year')['Month']):
                    for day in range(1, 32):
                        if day in list(new_orig_copy_df.query('Year == @year')['Day']):

                            path_df_new = 'C:/Users/Egor/Desktop/Oxigen_result/cutter/nst/'+f'{year}_{month}_{day}_nst.csv'
                            df_new = pd.read_csv(path_df_new, delimiter=',')

                            df_last = pd.concat([df_last, df_new])
                            df_last.to_csv('C:/Users/Egor/Desktop/Oxigen_result/joined/1930_6(New).csv', index=False)
